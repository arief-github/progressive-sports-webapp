            ...
            competition.standings.forEach((item) => {
                item.table.map((e) => {
                    ...
                    <div class="item-list w-full h-full py-[1px] grid gap-2 grid-cols-4 sm:grid-cols-6 md:grid-cols-8 lg:grid-cols-11 xl:grid-cols-11 2xl:grid-cols-12">
                        ...
                        <div class="w-full flex justify-start ${tampClass} col-span-2 truncate"><img class="w-10 h-10 mr-4" src="${e.team.crestUrl}" alt="${e.team.name}"> <a href="#/teams/${e.team.id}">${e.team.name}</a></div>
