...
const cardItemFavorite = ({nameTeam, location, stadion, pathImage, clubColor}) => {
	...
	const addColorsTeams = (colors) =>{
		...
		colors.forEach((e)=>{
			if(maxItem['start'] != maxItem['stop']){
				...
				item += `<div style="background-color : ${color}" class="w-[25px] h-[25px] rounded-[100%] my-[5px]"></div>`
			}
			maxItem['start']++;
		});
		return item;
	...
	return `
	  <div class="item-card w-full h-[400px] flex flex-col bg-[#f2f2f2] rounded-[8%] shadow-lg">
		  ...
		  <div class="side-bottom w-full h-1/6 mb-2 flex">
		  		<button class="btn-favorite rounded-[15%] w-[30%] h-5/6 bg-[#f9f9f9] mx-auto my-auto shadow-md">
